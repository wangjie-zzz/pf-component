...
function useWatcher() {
  ...
  // 更新列
  const updateColumns = () => {
    fixedColumns.value = _columns.value.filter((column: any) => column.fixed === true || column.fixed === "left");
    rightFixedColumns.value = _columns.value.filter((column: any) => column.fixed === "right");
    // @ts-ignore
  ...
  const toggleRowSelection = (row: AnyObject, selected: boolean, emitChange = true) => {
  ...
  const updateAllSelected = () => {
    ...
    let selectedMap: AnyObject;
    if (rowKey.value) {
      selectedMap = getKeysMap(selection.value, rowKey.value);
    }
    const isSelected = function(row: AnyObject) {
      if (selectedMap) {
  ...
  const updateSort = (column: null, prop: null, order: null) => {
    ...
    Object.keys(filters.value).forEach(columnId => {
      ...
      if (column && column.filterMethod) {
        sourceData = sourceData.filter(row => {
          return values.some((value: any) => column.filterMethod.call(null, value, row, column));
        });
      }
    });
  ...
  const clearFilter = (columnKeys: any[]) => {
    const { tableHeader, fixedTableHeader, rightFixedTableHeader } = instance.refs as AnyObject;
    let panels = {};
    ...
    if (Array.isArray(columnKeys)) {
      ...
      keys.forEach(key => {
        const column: any = columns.value.find((col: any) => col.id === key);
        if (column) {
          column.filteredValue = [];
        }
      });
  ...
  // 展开行与 TreeTable 都要使用
  const toggleRowExpansionAdapter = (row: AnyObject, expanded: boolean | undefined) => {
