...
function useStyle(props: TableHeaderProps) {
  ...
  const getHeaderCellStyle = (rowIndex: number, columnIndex: number, row: AnyObject, column: TableColumnCtx) => {
  ...
  const getHeaderCellClass = (rowIndex: number, columnIndex: number, row: AnyObject, column: TableColumnCtx) => {
    const classes = [column.id, column.order, column.headerAlign, column.className, column.labelClassName];
    if (rowIndex === 0 && isCellHidden(columnIndex, (row as unknown) as TableColumnCtx[])) {
