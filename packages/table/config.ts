...
export const cellForced = {
  selection: {
    ...
    renderCell: function({ row, column, store, $index }: { row: AnyObject; column: TableColumnCtx; store: Store; $index: string }) {
      return h(ElCheckbox, {
  ...
  index: {
    ...
    renderCell: function({ column, $index }: { column: TableColumnCtx; $index: string }) {
      let i = $index + 1;
...
export function defaultRenderCell({ row, column, $index }: { row: AnyObject; column: TableColumnCtx; $index: string }) {
