...
function useStyles(props: TableBodyProps) {
  ...
  const getCellStyle = (rowIndex: number, columnIndex: number, row: AnyObject, column: TableColumnCtx) => {
  ...
  const getCellClass = (rowIndex: number, columnIndex: number, row: AnyObject, column: TableColumnCtx) => {
  ...
  const getSpan = (row: AnyObject, column: TableColumnCtx, rowIndex: number, columnIndex: number) => {
    let rowspan = 1;
    let colspan = 1;
  ...
  const getColspanRealWidth = (columns: TableColumnCtx[], colspan: number, index: number) => {
    if (colspan < 1) {
